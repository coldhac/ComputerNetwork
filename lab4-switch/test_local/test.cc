...
int ClearProcessAndWaitExit(pid_t pid) {
  ...
  if (WIFSTOPPED(status)) std::cerr << "Signal" << std::endl;
  return -1;
}
...
static int send(int fd, char *buf, int len) {
  int times = 0, ret;
  while (times++ < 100 && (ret = write(fd, buf, len)) == -1) usleep(5000);
  return ret;
}

static int recv(int fd, char *buf) {
  int times = 0, ret;
  while (times++ < 100 && (ret = read(fd, buf, 9999)) == -1) usleep(5000);
  return ret;
}
